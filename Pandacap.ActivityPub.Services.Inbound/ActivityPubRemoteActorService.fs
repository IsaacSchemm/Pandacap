...
type ActivityPubRemoteActorService(
    ...
    interface IActivityPubRemoteActorService with
        ...
        member this.FetchAddresseeAsync(url: string, cancellationToken: CancellationToken) = task {
            if url = PublicCollection.Id then
                ...
                try
                    let! actor = (this :> IActivityPubRemoteActorService).FetchActorAsync(url, cancellationToken)
        ...
        member this.FetchAddresseesAsync(urls: string seq, cancellationToken: CancellationToken) =
            urls
            |> Seq.map (fun url -> (this :> IActivityPubRemoteActorService).FetchAddresseeAsync(url, cancellationToken))
            |> Task.WhenAll
